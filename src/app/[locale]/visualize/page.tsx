...
export default function VisualizePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const handleParse = async () => {
    if (!text.trim()) { setError(t.enterText); return; }
    ...
    try {
      ...
      else setError(result.error || t.parseFailed);
    } catch { setError(t.networkError); }
    setLoading(false);
  };

  const modCount = data?.modules?.length || 0;
  const reqCount = data?.modules?.reduce((a: number, m: any) => a + (m.requirements?.length || 0), 0) || 0;
  const missCount = data?.missingItems?.length || 0;
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <span className="text-gray-400 text-sm">{t.subtitle}</span>
        </div>
      </header>
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-500 text-sm">{t.totalCountBefore}{modCount}{t.totalCountMid1}{reqCount}{t.totalCountMid2}{missCount}{t.totalCountAfter}</p>
              </div>
              <button onClick={() => setData(null)} className="px-4 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50">{t.reUpload}</button>
            </div>
            ...
            {data.missingItems?.length > 0 && (
              <div className="bg-yellow-50 rounded-2xl border border-yellow-200 p-6">
                <h3 className="font-bold text-yellow-800 mb-3">{t.missingTitle}</h3>
