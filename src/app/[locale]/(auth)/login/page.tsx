...
export default function LoginPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.ok) window.location.href = data.user.role === 'admin' ? `/${locale}/admin` : `/${locale}`;
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-6">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 space-y-5">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">{t.password}</label><input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder={t.passwordPlaceholder} /></div>
          ...
          <p className="text-center text-sm text-gray-500">{t.noAccount} <a href={`/${locale}/register`} className="text-blue-600 hover:underline">{t.registerLink}</a></p>
        </form>
      </div>
    </div>
  );
